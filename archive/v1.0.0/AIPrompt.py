    ...
    def send_openai_prompt(self, model, user_prompt, api_key):
        ...
        if self.is_windows:
            system_prompt = (
                ...
                "1. \"powershell\": Include ONLY the exact PowerShell commands to execute. No comments, no explanations, "
                "no backticks, no markdown. Multiple commands should be separated by semicolons or newlines. "
                "If no command is needed, use empty string (\"\").\n\n"
                "2. \"zsh\": Always use empty string (\"\") since we're on Windows.\n\n"
                "3. \"instructions\": All other information goes here, including:\n"
                "   - Command explanations and descriptions\n"
                "   - Prerequisites or dependencies\n"
                "   - Expected output or behavior\n"
                "   - Error handling and troubleshooting\n"
                ...
                "   - Code examples and documentation\n"
                "   You may use Markdown formatting in this field only.\n\n"
                "4. \"title\": A concise, descriptive title (max 50 chars)\n\n"
                "Example response format:\n"
                "{\n"
                "  \"powershell\": \"Get-Process | Where-Object {$_.CPU -gt 10} | Select-Object Name, CPU\",\n"
                "  \"zsh\": \"\",\n"
                "  \"instructions\": \"This command lists all processes using more than 10% CPU...\",\n"
                "  \"title\": \"List High CPU Processes\"\n"
                "}"
            )
        else:
            system_prompt = (
                ...
                "and leverage ZSH's advanced features effectively.\n\n"
                "You MUST respond with a JSON object containing these exact keys:\n\n"
                "1. \"zsh\": Include ONLY the exact ZSH commands to execute. No comments, no explanations, "
                "no backticks, no markdown. Multiple commands should be separated by semicolons or newlines. "
                "If no command is needed, use empty string (\"\").\n\n"
                "2. \"powershell\": Always use empty string (\"\") since we're on Unix/macOS.\n\n"
                "3. \"instructions\": All other information goes here, including:\n"
                "   - Command explanations and descriptions\n"
                "   - Prerequisites or dependencies\n"
                "   - Expected output or behavior\n"
                ...
                "Example response format:\n"
                "{\n"
                "  \"zsh\": \"ps aux | awk '{if($3>10.0) print $0}'\",\n"
                "  \"powershell\": \"\",\n"
                "  \"instructions\": \"This command lists all processes using more than 10% CPU...\",\n"
                "  \"title\": \"List High CPU Processes\"\n"
                "}"
        ...
        payload = {
            ...
            "temperature": 0.7,  # Add some creativity while maintaining reliability
            "max_tokens": 1000
        }
        ...
        try:
            ...
            try:
                ...
                if not all(key in parsed for key in ["powershell", "zsh", "instructions", "title"]):
                    # Add missing fields with defaults
                    parsed = {
                        "powershell": parsed.get("powershell", ""),
                        "zsh": parsed.get("zsh", ""),
                        "instructions": parsed.get("instructions", "No additional instructions provided."),
                        "title": parsed.get("title", "Command Execution")
                    }
                ...
                return {
                    self.shell_key: "",
                    "instructions": "Error: Failed to parse AI response as JSON. Please try again.",
                    "title": "Error Response"
                }
        except Exception as e:
            print(f"Error sending OpenAI prompt: {e}")
            return None
