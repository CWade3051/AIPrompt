    ...
    def send_lm_studio_prompt(self, model, user_prompt):
        ...
        json_schema = {
            ...
            "schema": {
                ...
                "properties": {
                    "powershell": {
                        "type": "string",
                        "description": "PowerShell commands (if on Windows) or empty if not needed."
                    },
                    "zsh": {
                        "type": "string",
                        "description": "ZSH commands (if on macOS) or empty if not needed."
                    },
                    "instructions": {
                        "type": "string",
                        "description": "All additional context and instructions."
                    }
                },
                "required": ["powershell", "zsh", "instructions"]
            }
        }
        ...
        if self.is_windows:
            system_prompt = (
                ...
                "specialized in Windows development using PowerShell. For every response, "
                "output a JSON object with exactly three keys:\n\n"
                "• \"powershell\": Include any terminal command(s) that need to be executed on Windows PowerShell. "
                "These commands must be complete, precise, and tested to work on Windows. "
                "If no shell command is needed, output an empty string (\"\").\n\n"
                "• \"zsh\": Always output an empty string (\"\") since we're on Windows.\n\n"
                "• \"instructions\": Include all additional context, explanation, debugging steps, code examples, "
                "or any other instructions that would help a user understand the solution. "
                "You can use Markdown formatting in this field.\n\n"
                "Do not include any additional text outside this JSON structure. "
                "If tasks require running code in another language (e.g., Python), generate proper PowerShell "
                "commands to invoke that code. Always adhere to best coding practices and ensure that every "
                "command is safe and relevant to a Windows PowerShell environment."
            )
        else:
            system_prompt = (
                "You are a highly skilled coding, debugging, and troubleshooting assistant "
                ...
                "output a JSON object with exactly three keys:\n\n"
                "• \"zsh\": Include any terminal command(s) that need to be executed on a macOS ZSH terminal. "
                "These commands must be complete, precise, and tested to work on macOS. "
                "If no shell command is needed, output an empty string (\"\").\n\n"
                "• \"powershell\": Always output an empty string (\"\") since we're on macOS.\n\n"
                "• \"instructions\": Include all additional context, explanation, debugging steps, code examples, "
                "or any other instructions that would help a user understand the solution. "
                "You can use Markdown formatting in this field.\n\n"
                "Do not include any additional text outside this JSON structure. "
                "If tasks require running code in another language (e.g., Python), generate proper ZSH "
                "commands to invoke that code. Always adhere to best coding practices and ensure that every "
                "command is safe and relevant to a macOS ZSH environment."
            )
